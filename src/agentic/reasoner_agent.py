"""
Reasoner agent — handles analysis, comparison, and synthesis reasoning tasks.
"""
...
class ReasonerAgent(BaseAgent):
    ...
    def _determine_task_type(self, query: str, chunks: List) -> str:
        """
        Determine what type of reasoning task is needed.
    ...
    def _format_chunks(self, chunks: List, max_chunks: int = 10) -> str:
        """
    ...
    def _build_prompt(self, query: str, chunks: List, task_type: str) -> str:
        """
...
Answer:"""
    ...
    def _parse_response(self, content: str, task_type: str) -> Dict[str, Any]:
        """
    ...
    def execute(self, task: str, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Args:
            ...
            context: Dict with chunks and query keys.
        ...
        logger.debug("Determined task type: %s for query: %r", task_type, query[:50])

        prompt = self._build_prompt(query, chunks, task_type)

        import os
        if os.getenv('AGENTIC_SHOW_PROMPTS'):
            logger.info("\n%s\nREASONER PROMPT (%s):\n%s\n%s", "="*60, task_type, prompt, "="*60)
        ...
        try:
            ...
            if not content:
                logger.warning("Streaming failed: %s, using invoke as fallback", e)
                try:
                    response = self.llm.invoke(prompt)
                    content = response.content if hasattr(response, 'content') else str(response)
                except Exception as e2:
                    logger.error("Invoke also failed: %s, returning empty", e2)
        ...
        logger.info("Reasoning completed: task_type=%s  answer_length=%d", task_type, len(result["answer"]))
