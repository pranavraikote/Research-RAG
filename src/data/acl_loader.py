...
class ACLAnthologyLoader:
    ...
    def get_paper_metadata(self, paper_id):
        ...
        # Extracting title (MarkupText -> plain text)
        title = paper.title.as_text() if hasattr(paper.title, 'as_text') else str(paper.title)

        # Extracting authors (NameSpecification objects)
        authors = []
        ...
        # Extracting venue from venue_ids list
        venue_ids = getattr(paper, 'venue_ids', [])
        venue = venue_ids[0].upper() if venue_ids else None

        # Year is now a string in newer API
        ...
        # Extracting abstract (MarkupText -> plain text)
        abstract = None
        if hasattr(paper, 'abstract') and paper.abstract is not None:
            abstract = paper.abstract.as_text() if hasattr(paper.abstract, 'as_text') else str(paper.abstract)
    ...
    def list_papers_by_venue(self, venue, year = None, limit = None):
        ...
        # anthology.papers() is a generator in the newer API
        for paper in self.anthology.papers():

            # Checking venue match via venue_ids
            paper_venues = getattr(paper, 'venue_ids', [])
            venue_match = any(venue.lower() in v.lower() for v in paper_venues)

            if not venue_match:
                continue

            # Checking year match (year is a string in newer API)
            if year is not None:
                paper_year = getattr(paper, 'year', None)
                try:
                    paper_year_int = int(paper_year) if paper_year else None
                except (ValueError, TypeError):
                    paper_year_int = None
